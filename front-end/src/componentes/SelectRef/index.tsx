import React, { InputHTMLAttributes, useEffect, useRef, useState, useCallback, SelectHTMLAttributes } from "react"; // useRef vai buscar o valor de referencia dentro de um input ou de uma textarea
...
interface PropriedadesInput extends SelectHTMLAttributes<HTMLSelectElement>{
  name: string;
...
const SelectInput: React.FC<PropriedadesInput> = ({ name, icon: Icon, ...rest}) => {  //o useField precisa de uma referencia dentro do input para saber qual o campo ele vai tratar, etou usando o "name", e o ...rest é para pegar qualquer tipo de informação que vem do input tipo password, email, etc
  const inputValue = useRef<HTMLSelectElement>(null); // por causa do Typescript preciso informar q o useRef vai usar as propriedades do <HTMLInputElement>
  ...
  return(
  <Container className="BordaSelectRef" campoErradoEstilo={!!error} focoinputEstilo={focoinput} > {/*preenchidoinput={preenchidoinput}*/}  {/* focoinput precisa ser declarado no style na interface e falar que o Container usa a interface*/}
   {Icon && <Icon size={20} />} 
    <select     
    ...
    { error && (
      <Errodiv title={error}>  {/*no estilo do CSS eu importei o Tooltip e ele fala q precisa de um title, o title dele vai ser o erro que vai chegar nele exemplo "campo obrigatorio" */}
        <FiAlertCircle color='red'/> 
      </Errodiv>
    )}  {/* se tiver um error preenchi do que vem la do input && então faça isso coloca o icone */}
  </Container>
  );
}
